...
=====================================================================================
  ...
  WEB UI  http://motorcontroller.local/   (mDNS — works no matter what IP DHCP assigns)
  ─────────────────────────────────────────────────────────────
...
// ── mDNS hostname ─────────────────────────────────────────────────────
// Device is reachable at http://motorcontroller.local/ no matter what IP
// the router's DHCP server hands it. Change this string for a different
// name (letters/digits/hyphens only, no spaces, no dots).
...
<div class="phone">
  ...
  <!-- footer -->
  <div class="footer">
    <div class="footer-ip" id="footerHost">motorcontroller.local</div>
...
void setup() {
  ...
  // WiFi — DHCP (dynamic IP). No WiFi.config() call, so the router assigns
  // whatever IP is free; mDNS below makes that IP irrelevant to the user.
  WiFi.mode(WIFI_STA);
  ...
  // mDNS — advertises this device as http://<MDNS_HOSTNAME>.local/
  // regardless of whatever DHCP IP it ends up with. Re-attempted on every
  // reconnect in loop() since a dropped WiFi link kills the mDNS responder.
...
void loop() {
  ...
  if (connected && g_wifiState != WF_CONNECTED) {
    setWifiState(WF_CONNECTED);
    // Re-announce mDNS after a reconnect — the responder doesn't survive
    // a dropped link, so it needs to be restarted alongside the socket.
